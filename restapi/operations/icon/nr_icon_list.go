...
func (o *NrIconList) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	db.Table("icons").Select("icons.id, icons.name,icons.cover,icons.jumpid,icons.webUrl,icons.type").Joins("left join category_icon_relation on icons.id = category_icon_relation.iconId").Where("category_icon_relation.categoryId =?",*Params.PosID).Where("icons.status=?",0).Where("category_icon_relation.status=?",0).Find(&icons)
	//query
